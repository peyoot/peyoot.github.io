import{_ as e,o as i,c as n,b as d}from"./app-CyV6FjEB.js";const l={},s=d(`<h1 id="在dey-aio环境下使用devtool进行驱动开发和调试" tabindex="-1"><a class="header-anchor" href="#在dey-aio环境下使用devtool进行驱动开发和调试"><span>在DEY AIO环境下使用devtool进行驱动开发和调试</span></a></h1><p>本文旨在帮助对嵌入式系统不熟悉的用户熟悉和了解驱动程序的相关开发知识。虽然可以直接在yocto项目的tmp目录下找到相关的源代码并用bitbake -c devshell virtual/kernel来进入配置好环境的终端进行编译和测试，但这种方式需要手动管理源代码路径、生成补丁，并可能导致 tmp/work 目录混乱。这对需要迭代开发非常不友好，因此一般还是要使用 devtool工具并结合bitbake相关命令来开发和修改 Yocto 项目中的内核源代码（如驱动补丁，设备树等）。devtool 有以下优势：</p><p>自动化管理：它会自动从 tmp/work 提取源代码到项目 workspace 的 sources/ 目录下，提供一个干净的开发环境。你可以像本地编辑一样修改文件，而不用担心构建目录的临时性。 补丁生成：修改后，直接用 devtool update-recipe 生成补丁，并自动更新内核配方（recipe），无需手动 quilt 或 git 格式化补丁。 构建隔离：修改只影响当前 workspace，不会污染全局构建，直到你选择更新配方。 易回滚：如果出问题，可以轻松重置或移除修改。 集成性：适合迭代开发，尤其在内核驱动如 OV2740.c 的场景下。</p><p>不过，devtool 也依赖于你的 Yocto 环境正确配置（bitbake 已初始化，source 了环境脚本）。在 Digi Embedded Yocto (DEY) 项目中，它应该无缝工作，因为 DEY 基于标准 Yocto。</p><h1 id="完整示例" tabindex="-1"><a class="header-anchor" href="#完整示例"><span>完整示例</span></a></h1><p>以下是一个完整流程，请确保是在workspace下的项目目录里操作，比如workspace/myccmp25/，</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code># 0. 确保环境
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h1 id="linux配方bbappend" tabindex="-1"><a class="header-anchor" href="#linux配方bbappend"><span>linux配方bbappend</span></a></h1><p>早期设备树拷贝install_dts 被挂在 do_patch:append 里,也就是在 kernel-yocto 的 do_patch 主体(它会用 kern-tools 建 git 历史并把源码树 commit 掉)之后,把 ccmp25-softlink.dts 拷进，这会导致devtool modify的 post-patch 逻辑会把这份 untracked dts 收进 devtool 分支。从而使得这个命令执行出错，把这个安装动作挪到 do_patch 之后、do_configure 之前就行。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>addtask install_dts after do_patch before do_configure
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div>`,16),a=[s];function c(t,v){return i(),n("div",null,a)}const r=e(l,[["render",c],["__file","devtool.html.vue"]]),m=JSON.parse('{"path":"/zh/deyaio/wiki/devtool.html","title":"在DEY AIO环境下使用devtool进行驱动开发和调试","lang":"zh-CN","frontmatter":{"description":"在DEY AIO环境下使用devtool进行驱动开发和调试 本文旨在帮助对嵌入式系统不熟悉的用户熟悉和了解驱动程序的相关开发知识。虽然可以直接在yocto项目的tmp目录下找到相关的源代码并用bitbake -c devshell virtual/kernel来进入配置好环境的终端进行编译和测试，但这种方式需要手动管理源代码路径、生成补丁，并可能导致 ...","head":[["meta",{"property":"og:url","content":"https://peyoot.github.io/zh/deyaio/wiki/devtool.html"}],["meta",{"property":"og:title","content":"在DEY AIO环境下使用devtool进行驱动开发和调试"}],["meta",{"property":"og:description","content":"在DEY AIO环境下使用devtool进行驱动开发和调试 本文旨在帮助对嵌入式系统不熟悉的用户熟悉和了解驱动程序的相关开发知识。虽然可以直接在yocto项目的tmp目录下找到相关的源代码并用bitbake -c devshell virtual/kernel来进入配置好环境的终端进行编译和测试，但这种方式需要手动管理源代码路径、生成补丁，并可能导致 ..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:locale","content":"zh-CN"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"在DEY AIO环境下使用devtool进行驱动开发和调试\\",\\"image\\":[\\"\\"],\\"dateModified\\":null,\\"author\\":[]}"]]},"headers":[],"git":{},"autoDesc":true,"filePathRelative":"zh/deyaio/wiki/devtool.md"}');export{r as comp,m as data};
