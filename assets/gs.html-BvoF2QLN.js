import{_ as e,o as i,c as n,b as s}from"./app-nyDMWEXJ.js";const l={},d=s(`<h1 id="安装" tabindex="-1"><a class="header-anchor" href="#安装"><span>安装</span></a></h1><hr><p>为了正确安装和使用dey-aio，您需要安装yocto开发环境所需的依赖包，如果您要使用docker进行开发，则还需要安装docker和docker-compose。 强烈建议您使用一台Linux服务器来安装deyaio开发环境。由于编译自定义的Linux镜像，特别是第一次编译耗时较长，尽量避免使用笔记本或日常工作的电脑来安装DEY开发环境。使用服务器来安装开发环境，在开发时通过日常工作所用的电脑 SSH远程登陆的方式来进行配置和编译，结合tmux工具让编译在SSH Session退出或关闭的情况下不中断编译过程，从而获得更好的系统编译开发体验。 下面安装过程以Ubuntu 22.04为例，同样也适用于Ubuntu 20.04，请使用普通用户来执行这些命令。</p><p>由于国内github访问经常被间歇式阻断，而编译过程中需要流畅的github访问，建议使用PVPN自行搭建科学上网的环境，以确保编译不受GFW防火墙的干扰。</p><h2 id="安装必要的依赖包" tabindex="-1"><a class="header-anchor" href="#安装必要的依赖包"><span>安装必要的依赖包</span></a></h2><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>sudo apt update
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>阅读DEY的开源声明，空格键翻页，最后输入y确认。</p><p>然后到conf/local.conf中，修改它的内容为meta-custom下homeaddons配方里提供的.local.conf中的内容</p><p><em><strong><strong><strong><strong><strong><strong>以下不用看</strong></strong></strong></strong></strong></strong></em>********</p><p>在conf/local.conf中设置实时选项，添加或移除包， 实时开启：</p><p>如果需要对镜像做裁减和增加不同包，可以用： IMAGE_INSTALL:append = &quot; 包名&quot; 和 IMAGE_INSTALL:remove = &quot; 包名&quot; 来实现。 大而全的配置可以用下面这个，然后再裁减：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>DISTRO_FEATURES:append = &quot; rt opengl pam  hwlatdetect&quot;
