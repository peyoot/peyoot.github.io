import{_ as e,o as i,c as n,b as s}from"./app-Gzti1wGm.js";const a={},d=s(`<h1 id="离线编译dey项目" tabindex="-1"><a class="header-anchor" href="#离线编译dey项目"><span>离线编译DEY项目</span></a></h1><p>DEY项目的编译并不总是需要联网。Yocto项目的编译受益于开源项目，通常也建议保持更新以使用最新的上游源码，这对于获取一些较新的软件包或驱动来说至关重要。不过对于工业项目来说，稳定可靠，可重复编译出原始的镜像，这也很重要。本章节我们将讨论如何锁定DEYAIO编译出来的项目版本，并支持离线编译，以便在内网环境中持续开发项目。</p><h2 id="锁定dey项目中manifest仓库指定的各软件库的git版本" tabindex="-1"><a class="header-anchor" href="#锁定dey项目中manifest仓库指定的各软件库的git版本"><span>锁定DEY项目中manifest仓库指定的各软件库的git版本</span></a></h2><p>首先检查编译所使用的软件仓库，DEY项目通过repo管理整个项目用到的软件包。DEY AIO的manifest仓库定义了上游软件库的git版本，在DEY项目根目录我们可以通过</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>cat .repo/manifest.xml
...
&lt;/manifest&gt;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>表明当前的manifest仓库是isp.xml。结合下面</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ cd .repo/manifests
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>它表明当初用的是初始化项目用的是：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>repo init -u https://github.com/peyoot/dey-aio-manifest.git -b refs/tags/dey5.0-r3 -m isp.xml
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>而打开.repo/manifests/isp.xml，就可以看到各个project拉取的git分支。其中revision是某个提交的hash值的，表明是锁定版本，而如果是像scarthgap之类的yocto版本号，则表明这个项目的版本取决于上游对应该版本号的最新提交。为了确保离线编译，我们可以先创建一个锁定版本my_fronzon_isp.xml，并切换到这个manifest仓库：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>repo manifest -o .repo/manifests/my_frozen_isp.xml -r --suppress-upstream-revision --suppress-dest-branch
repo init -m my_frozen_isp.xml
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="联网状态下完成一次完整的编译" tabindex="-1"><a class="header-anchor" href="#联网状态下完成一次完整的编译"><span>联网状态下完成一次完整的编译</span></a></h2><p>上面操作后，我们检出了锁定版本的isp.xml，这是一个各层配方均有锁定版本的manifest仓库，我们需要在联网状态下完成一次编译，以便在编译过程中下载相应的源码，作为后续内网编译或本机编译时使用。这一步骤和正常的的DEY项目完全一样，不过为了方便后续内部机器通过内网编译，需要在创建好项目后，修改conf/local.conf，在最后处添加一句：BB_GENERATE_MIRROR_TARBALLS = &quot;1&quot; 完整过程</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>cd dey5.0/workspace
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>注，如果因网络问题始终无法完成一次完整编译时，检查每次报错时所用的配方，先</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>bitbake -c cleansstate 出错配方名
bitbake dey-image-lvgl
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>直到最终编译完成，上面编译镜像也可用bitbake -k dey-image-lvgl以加快速度。 如果确实无法编译完成，请向Digi官方索取相关的编译环境压缩包，并用后文内网编译的方式实现离线编译。</p><h2 id="本机离线编译配置" tabindex="-1"><a class="header-anchor" href="#本机离线编译配置"><span>本机离线编译配置</span></a></h2><p>在conf/local.conf中添加下面关键设置：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code># 指定源码URL镜像，请替换为您的实际IP和端口和本机downloads目录
SOURCE_MIRROR_URL ?= &quot;\\
...
# 禁掉网络，仅本机编译，确保完全离线构建
BB_NO_NETWORK = &quot;1&quot;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="内网编译" tabindex="-1"><a class="header-anchor" href="#内网编译"><span>内网编译</span></a></h2><p>内网编译是指不访问公网github，仅使用内部局域网实现多台机器编译DEY项目。如果仅是本机单机离线编译，参考上面的内容就足够了。如果需要实现内网任意机器共用这个源码来编译或开发，请继续配置。</p><h3 id="设置downloads和sstate-cache目录在内网共享" tabindex="-1"><a class="header-anchor" href="#设置downloads和sstate-cache目录在内网共享"><span>设置downloads和sstate-cache目录在内网共享</span></a></h3><p>DEY AIO项目默认会把下载的构件和缓存放在workspace/project_shared的相应目录下，我们需要设置HTTP服务器共享，以便内网其它机器可以访问。 这实际上是要配置一个web服务器，并且可以访问文件系统目录，有很多实现方法，建议以nginx来搭建，不过如果寻求快速，也可以用python。 方法一：用python命令快速搭建web服务器： 以python为例，进入workspace/project_shared目录后，执行：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>python3 -m http.server 8000
