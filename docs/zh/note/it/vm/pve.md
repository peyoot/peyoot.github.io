...
把 /var/lib/docker、/tmp、/home 和 / 作为四个独立分区，并实现开机自动挂载，要正确配置 /etc/fstab 文件。

分区布局：
系统盘60G,添加下面额外盘，新创建系统也一样，相同挂载回去就可以用，即使系统盘破坏了也没关系。 
/var/lib/docker 360G 
...
由于用ZFS，这些选项默认可以用：
1、Discard 
务必勾选：这个选项允许虚拟机向PVE宿主发送 TRIM/Discard 指令。对于ZFS至关重要：当您在虚拟机内删除文件或格式化时，勾选此选项后，这些操作会通知到底层的ZFS存储。ZFS可以据此回收这些不再使用的数据块空间，避免虚拟磁盘文件（ZVOL）持续膨胀，占用不必要的存储池空间。虚拟机内的操作系统也需要支持并启用TRIM（对于Ubuntu，默认安装的ext4文件系统已支持，无需额外设置）。
2、cache
如果是性能至上，还可以把Cache改为write back，不过且ZFS池本身需配置了冗余（如RAIDZ1/2，Mirror），这样才能修复断电数据没及时写入的错误。如果是单机raid0就不要动它，用默认即可。

### DNS相关设置
一般search domain可以用本机型号或本机位置来区别，比如hp850g.sho，这样就方便用它作为本地域名来操作。DNS用公网的即可。
